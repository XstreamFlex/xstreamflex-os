/**
 * XMail Client SDK for Xsites, EZsites & XMG Ecosystem
...
(function () {
  ...
  const API_ENDPOINT = currentScript ? currentScript.getAttribute('data-api-url') || currentScript.src.replace('/sdk/xmail.js', '/api/v1/events/ingest') : '/api/v1/events/ingest';
  const ECOSYSTEM_ENDPOINT = API_ENDPOINT.replace('/events/ingest', '/ecosystem');
  ...
  window.XMail = {
    ...
    getIdentity: function () {
      ...
      const url = this.ecosystemEndpoint + '/identity/' + encodeURIComponent(this.siteKey);
      return fetch(url)
        .then(function (res) { return res.json(); })
